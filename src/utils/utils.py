...
def read_and_process_to_trigram_vecs(data_files, data_path='../data/raw/', sample_size=100, test_split=0.0, concat=True):
  ...
  train_trigram_vecs, train_trigram_idxs = process_years(train_strains_by_year, concat, trigram_to_idx, trigram_vecs_data)
  test_trigram_vecs, test_trigram_idxs = process_years(test_strains_by_year, concat, trigram_to_idx, trigram_vecs_data)

  return train_trigram_vecs, test_trigram_vecs, train_trigram_idxs, test_trigram_idxs

def process_years(strains_by_year, concat, trigram_to_idx, trigram_vecs_data):
